...
public abstract class MyBeanUtils extends org.springframework.beans.BeanUtils {
		...
		for (PropertyDescriptor targetPd : targetPds) {
			...
			if (sourcePd != null && sourcePd.getReadMethod() != null) {
				try {
					...
					if(s_value instanceof String && s_value.toString() .equals("")){
						s_value = null;
					}
					if (s_value != null) {
	...
	public static void saveProperties(Properties p, String name) {
		try {
			URL url = MyBeanUtils.class.getClassLoader().getResource("/");
	...
	public static Properties loadProperties(String name) {
		InputStream is = MyBeanUtils.class.getClassLoader().getResourceAsStream(name);
