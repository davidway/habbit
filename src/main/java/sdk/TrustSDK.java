...
public class TrustSDK {
	 ...
	 * @since JDK 1.7
	 */
	public static PairKey generatePairKey(boolean encode) throws TrustSDKException {
		try {
			PairKey pair = new PairKey();
			String privateKey = ECDSAAlgorithm.generatePrivateKey();
			String pubKey = ECDSAAlgorithm.generatePublicKey(privateKey.trim(), encode);
	 ...
	 * @author ronyyang
	 * @param prvKey
	 *            输入 存放私钥 长度必须为PRVKEY_DIGEST_LENGTH
	 * @param pubKey
	 *            输入 存放公钥 长度必须为PUBKEY_DIGEST_LENGTH
	...
	public static String generateAddrByPrvkey(String privateKey) throws TrustSDKException {
		...
		try {
			String pubKey = ECDSAAlgorithm.generatePublicKey(privateKey, true);
	 ...
	 * @since JDK 1.7
	 */
	public static String signString(String privateKey, byte[] data, boolean isHash256) throws TrustSDKException {
		if (StringUtils.isEmpty(privateKey)) {
			throw new TrustSDKException(ErrorNum.INVALID_PARAM_ERROR.getRetCode(),
					ErrorNum.INVALID_PARAM_ERROR.getRetMsg());
		}
		try {
			String result = ECDSAAlgorithm.sign(privateKey, data, isHash256);
			return result;
		} catch (Exception e) {
	 ...
	 * @since JDK 1.7
	 */
	public static boolean verify(String pubKey, byte[] bytes, boolean isHash256, String sign) throws TrustSDKException {
		try {
			return ECDSAAlgorithm.verify(bytes, isHash256, sign, pubKey);
		} catch (Exception e) {
			throw new TrustSDKException(ErrorNum.ECDSA_ENCRYPT_ERROR.getRetCode(),
					ErrorNum.ECDSA_ENCRYPT_ERROR.getRetMsg(), e);
		}
	...
	public static byte[] encryptDES3(String key, byte[] data) throws TrustSDKException {
		...
		try {
			...
			throw new TrustSDKException(ErrorNum.DES3_ENCRYPT_ERROR.getRetCode(),
					ErrorNum.DES3_ENCRYPT_ERROR.getRetMsg(), e);
		}
	}
	...
	public static byte[] encryptAES128(String key, byte[] data) throws TrustSDKException {
		...
		try {
			...
			throw new TrustSDKException(ErrorNum.AES_ENCRYPT_ERROR.getRetCode(), ErrorNum.AES_ENCRYPT_ERROR.getRetMsg(),
					e);
		}
	}
	 ...
	 * @since JDK 1.7
	 */
	// public static String issSign(String infoKey, String infoVersion, int state,
	// String content, String notes,
	// String commitTime, String privateKey) throws TrustSDKException {
	// if (StringUtils.isEmpty(infoKey) || StringUtils.isEmpty(infoVersion) ||
	// StringUtils.isEmpty(content)
	// || StringUtils.isEmpty(notes) || StringUtils.isEmpty(commitTime) ||
	...
	// String contentStr =
	// SignStrUtil.mapToKeyValueStr(SignStrUtil.jsonToMap(contentJson));
	// notes = notes.replaceAll("\r", "\\\\\\\\r").replaceAll("\n", "\\\\\\\\n");
	// JSONObject noteJson = JSONObject.parseObject(notes);
	// String noteStr =
	// SignStrUtil.mapToKeyValueStr(SignStrUtil.jsonToMap(noteJson));
	...
	// String result = signString(privateKey, srcStr.getBytes("UTF-8"), false);
	// return result;
	// } catch (Exception e) {
	// throw new TrustSDKException(ErrorNum.GENERATE_SIGN_ERROR.getRetCode(),
	// ErrorNum.GENERATE_SIGN_ERROR.getRetMsg(), e);
	// }
	// }

	public static String SignRenString(String privateKey, byte[] data) throws TrustSDKException {
		if (StringUtils.isEmpty(privateKey) || data == null) {
			throw new TrustSDKException(ErrorNum.INVALID_PARAM_ERROR.getRetCode(),
					ErrorNum.INVALID_PARAM_ERROR.getRetMsg());
		}
		try {
			String result = signString(privateKey, data);
			return result;
		} catch (Exception e) {
			throw new TrustSDKException(ErrorNum.GENERATE_SIGN_ERROR.getRetCode(),
					ErrorNum.GENERATE_SIGN_ERROR.getRetMsg(), e);
	 ...
	 * @since JDK 1.7
	 */
	public static boolean issVerifySign(String infoKey, String infoVersion, int state, String content, String notes,
			String commitTime, String pubkey, String sign) throws TrustSDKException {
		if (StringUtils.isEmpty(infoKey) || StringUtils.isEmpty(infoVersion) || StringUtils.isEmpty(content)
				|| StringUtils.isEmpty(notes) || StringUtils.isEmpty(commitTime) || StringUtils.isEmpty(pubkey)
		...
		try {
			...
			throw new TrustSDKException(ErrorNum.VERIFY_SIGN_ERROR.getRetCode(), ErrorNum.VERIFY_SIGN_ERROR.getRetMsg(),
					e);
		}
	 ...
	 * @return
	 */
	public static boolean verifyRenSign(String pubKey, byte[] signSrc, String sign) throws TrustSDKException {
		if (StringUtils.isEmpty(pubKey) || signSrc == null || StringUtils.isEmpty(sign)) {
			throw new TrustSDKException(ErrorNum.INVALID_PARAM_ERROR.getRetCode(),
					ErrorNum.INVALID_PARAM_ERROR.getRetMsg());
		}
		try {
			boolean result = ECDSAAlgorithm.VerifyRenSign(pubKey, signSrc, sign);
			return result;
		} catch (Exception e) {
			throw new TrustSDKException(ErrorNum.VERIFY_SIGN_ERROR.getRetCode(), ErrorNum.VERIFY_SIGN_ERROR.getRetMsg(),
					e);
	 ...
	 * @since JDK 1.7
	 */
	public static String generateTransSql(String fseqno, String privateKey, String fdst1, double fdst1_amount,
			String fdst2, double fdst2_amount, String fassets, String fattach, String ftime) throws TrustSDKException {
		...
		try {
			...
			treeMap.put("Fpubkey", pubKey);
			String srcStr = SignStrUtil.mapToKeyValueStr(treeMap);
			String sign = signString(privateKey, srcStr.getBytes("UTF-8"), false);
			String result = String.format(
					"insert into t_transaction (Fseqno,Fsrc,Fdst1,Fdst1_amount,Fdst2,Fdst2_amount,Ftime,Fpubkey,Fsign,Fassets,Fattach) values ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s');",
					fseqno, addr, fdst1, fdst1_amount, fdst2, fdst2_amount, ftime, pubKey, sign, assetsStr, attachStr);
			return result;
		} catch (Exception e) {
			throw new TrustSDKException(ErrorNum.GENERATE_SQL_ERROR.getRetCode(),
					ErrorNum.GENERATE_SQL_ERROR.getRetMsg(), e);
		}
	}
